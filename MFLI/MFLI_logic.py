...
class MFLI_Logic(QtCore.QThread):
    ...
    def configure_basic_mode_hardware(self):
        ...
        try:
            ...
            for osc_index in [1, 2, 3]:
                self.hardware.set_osc_amplitude(osc_index = osc_index, amplitude = 0)
                self.hardware.set_osc_frequency(osc_index = osc_index, f_hz = 1000)
                self.hardware.set_osc_output_enable(osc_index = osc_index, enable = False)
    ...
    def get_frequency1(self, osc_index=0):
        if self.hardware is None:
            raise RuntimeError("Hardware not connected")
        try:
            val = self.hardware.get_osc_frequency(osc_index)
    ...
    def set_frequency1(self, osc_index=0):
        ...
        try:
            ...
            self.hardware.set_osc_frequency(osc_index, freq)
    ...
    def get_amplitude1(self, osc_index=0):
        if self.hardware is None:
            raise RuntimeError("Hardware not connected")
        try:
            val = self.hardware.get_osc_amplitude(osc_index)
    ...
    def set_amplitude1(self, osc_index=0):
        ...
        try:
            ...
            self.hardware.set_osc_amplitude(amp, osc_index)
    ...
    def get_X(self):
        if self.hardware is None:
            raise RuntimeError("Hardware not connected")
        try:
            data = self.hardware.get_demod_sample()
            val = data['x']
            self.sig_X.emit(val)
            return val
        except (MFLIHardwareError, Exception) as e:
    ...
    def get_Theta(self):
        ...
        try:
            ...
            self.sig_is_changing.emit(f"Error getting Theta: {e}")
            raise 
