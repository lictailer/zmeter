...
It is registered under driver ID `keithley24xx` with required connection field
`address`. For first commissioning keep `connect_on_start=false`; a later
reviewed startup request schedules the existing asynchronous panel connection
and reports pending without waiting for completion. The panel remains
authoritative for the final result and manual retry. Runtime mutation remains
disabled.
