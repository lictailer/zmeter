...
Do not assume a vendor transport is thread-safe. If a device can also be used by monitoring or UI jobs, its integration must prevent contention. Before the scan worker starts, `Scan` seals editable settings and captures the physical devices referenced by executable setters, getters, averaged-getter sources, and manual before/after writes. It stops GUI-owned monitors on the Qt owner thread, then calls `stop_scan()` for that immutable participating set and initializes NumPy result arrays from a copied level model on the bounded scan-boundary worker. Only successful preparation returns to the GUI thread for plot setup and `ScanLogic.start()`; a failure restores the same set before releasing the runtime reservation. Plot-only and default channels do not participate; an artificial write expands to both configured physical backing devices, while an artificial getter alone does not.
