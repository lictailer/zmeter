...
Hardware-independent syntax check: `python -B -m py_compile nidaq/nidaq_hardware.py nidaq/nidaq_logic.py nidaq/nidaq_main.py`. Focused regression check: `python -B -m unittest tests.test_nidaq_two_ao -v`. No general simulated PyDAQmx backend is present.
